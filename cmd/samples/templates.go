...
const envoyConfigTemplate = `admin:
				...
				route_config:
					...
					- name: default
						...
						- match: { prefix: /httpbin }
							route:
								cluster: {{target-service-name}}
							typed_per_filter_config:
								envoy.filters.http.dynamic_forward_proxy:
									"@type": type.googleapis.com/envoy.extensions.filters.http.dynamic_forward_proxy.v3.PerRouteConfig
									host_rewrite_literal: {{target-service}}  # rewrites :authority for Apigee product check
...
- name: {{target-service-name}}
	...
	load_assignment:
		cluster_name: {{target-service-name}}
		endpoints:
		- lb_endpoints:
			- endpoint:
					address:
